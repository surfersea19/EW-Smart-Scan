...
class SchedulerAdapter:
    ...
    def __init__(
        self,
        p2_scheduler,
        max_history_per_band: int = 200,
    ):
        self.p2_scheduler = p2_scheduler
        self._max_history = max_history_per_band
        self._hm = BandHistoryManager(
            max_history_per_band=max_history_per_band
        )
        self._ingested = 0

        # Diagnostic/display-only state.
        # Never fed back into decision logic.
        self.last_predictions: dict = {}
        self.last_breakdown: list = []
        self.last_reason: Optional[str] = None
        self.last_behavior: Optional[str] = None
        self.last_behavior_confidence: float = 0.0

    def reset(self) -> None:
        """Reset adapter and delegated scheduler state."""

        self._hm = BandHistoryManager(
            max_history_per_band=self._max_history
        )
        ...
        self.last_behavior_confidence = 0.0
    ...
    def choose_band(
        ...
        self._ingest_new_observations(
            observation_log
        )

        bands = spectrum.list_bands()

        prev_last_scanned = getattr(
            self.p2_scheduler,
            "_last_scanned",
            None,
        )

        chosen = self.p2_scheduler.select_band(
            bands,
            self._hm,
            current_time=t,
        )
    ...
    def _ingest_new_observations(
        self,
        observation_log: list,
    ) -> None:
        # observation_log is P1's full history to date;
        # only translate and ingest what's new.
        from .observation_adapter import (
            p1_observation_to_p2_dict,
        )
    ...
    def _update_reasoning(
        ...
        if (
            ...
            # SequentialScheduler / RandomScheduler /
            # future non-ML schedulers.
            self.last_predictions = {}
            self.last_breakdown = []
            self.last_reason = (
                self._describe_non_ml_scheduler()
            )
            return

        try:
            predictions = (
                predictor.predict_all_bands(
                    bands,
                    self._hm,
                    t,
                )
            ...
            # select_band() already advanced _last_scanned
            # to the chosen band. Restore the previous value
            # temporarily so score reconstruction matches the
            # real decision-time state.
            if has_last_scanned:
                self.p2_scheduler._last_scanned = (
                    prev_last_scanned
                )

            behavior_detector = getattr(
                self.p2_scheduler,
                "behavior_detector",
                None,
            )
            ...
            for band, prob in predictions.items():
                ...
                # Phase 7 behavior diagnostics.
                #
                # This uses only the receiver-observation
                # history stored in BandHistoryManager.
                if behavior_detector is not None:
                    try:
                        behavior_result = (
                            behavior_detector.analyze(
                                self._hm.get_band_history(
                                    band
                                ),
                                current_time=t,
                            )
                        ...
                        # Diagnostic only.
                        # Never allow behavior-analysis failure
            ...
            # select_band()'s non-exploration branch
            # selects the argmax score.
            top = (
                breakdown[0]
                if breakdown
                else None
            ...
            if explored:
                ...
                if chosen_row:
                    behavior = chosen_row.get(
                        "behavior",
                        "unknown",
                    )

                    confidence = chosen_row.get(
                        "behavior_confidence",
                        0.0,
                    )
                    self.last_behavior = behavior
                    self.last_behavior_confidence = confidence

                    if behavior != "unknown":
                        self.last_reason = (
                            f"predicted activity "
                            f"{chosen_row['probability']:.0%}, "
                            f"behavior {behavior} "
                            f"({confidence:.0%}), "
                            f"score "
                            f"{chosen_row['score']:.2f}"
                        )
                    else:
                        self.last_reason = (
                            f"predicted activity "
            ...
            # Reasoning/display is best-effort only.
            # A failure here must NEVER surface as a failure
            # of the live scheduling decision.
            self.last_predictions = {}
            self.last_breakdown = []
            self.last_reason = (
                "smart scheduler decision"
    ...
    def _describe_non_ml_scheduler(
        self,
    ) -> str:
        name = type(
            self.p2_scheduler
